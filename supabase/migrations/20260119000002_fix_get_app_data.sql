-- Fix get_app_data function with proper subqueries for ordering
...
BEGIN
  ...
  RETURN json_build_object(
    ...
    'categories', COALESCE((
      ...
      FROM (SELECT * FROM categories ORDER BY sort_order) c
    ), '[]'::json),
    ...
    'interview_stages', COALESCE((
      SELECT json_agg(row_to_json(s))
      FROM (SELECT * FROM interview_stages ORDER BY order_index) s
    ), '[]'::json),
