-- Fix activity_log to include mentor name for display
-- The issue: activity_log.mentor_id may not match any mentor (e.g., deleted mentors)
-- Solution: Join with mentors table and include mentor_name in the result
...
BEGIN
  ...
  RETURN json_build_object(
    ...
    'activity_log', COALESCE((
      SELECT json_agg(row_to_json(activity_with_mentor))
      FROM (
        SELECT
          a.id,
          a.mentor_id,
